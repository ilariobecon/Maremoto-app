...
def fetch_tides(spot_name):
    # Simulazione marea diversa per spot (basata su nearest_port o nome)
...
# -------------------------------- GENERA REPORT -------------------------------
def generate_fishing_info(spot, selected_date, start_city, is_premium):
...
1. {spot['esche'][0]} ({spot['prede'].get(next(iter(spot['prede'])), 40)}% principale)  
2. {spot['esche'][1]}  
3. {spot['esche'][2]}  
Prede principali: {', '.join([f"{k} {v}%" for k, v in list(spot['prede'].items())[:3]])}  
...
if not df_spots.empty:
    ...
    if selected_name:
        ...
        report = generate_fishing_info(spot, selected_date, start_city, is_premium)
        st.markdown(report)
